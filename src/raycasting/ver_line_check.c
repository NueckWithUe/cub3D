...
/*   Updated: 2024/10/30 16:03:39 by nnagel           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
float	vertical_line_check(t_data *data)
{
	...
	{
		data->ray->r_x = (((int)(data->player->pos_x / 64)) + 1) * 64;
		data->ray->o_x = 64;
		data->ray->r_y = data->player->pos_y + (data->ray->r_x - data->player->pos_x) * tanf(data->ray->angle);
		data->ray->o_y = 64 * tanf(data->ray->angle);
	}
	else if (data->ray->angle > (M_PI / 2) && data->ray->angle < 3 * (M_PI / 2))
	{
		data->ray->r_x = ((int)(data->player->pos_x / 64)) * 64 - __FLT_EPSILON__;
		data->ray->o_x = -64;
		data->ray->r_y = data->player->pos_y + (data->ray->r_x - data->player->pos_x) * tanf(data->ray->angle);
		data->ray->o_y = -64 * tanf(data->ray->angle);
	}
